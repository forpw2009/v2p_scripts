...
def bark(title, content):
    ...
    if not BARK:
        ...
        return
    print("bark服务启动")
    response = requests.get("https://api.day.app/{brr}/{title}/{content}".format(brr = BARK,title=title,content =content).json())
...
def serverJ(title, content):
    ...
    response = requests.post("https://sc.ftqq.com/{}.send".format(SCKEY), data=data).json()
    if response['errno'] == 0:
        print('推送成功！')
    else:
        print('推送失败！')
...
def telegram_bot(title, content):
    ...
    if not bot_token or not user_id:
        ...
        return
    print("tg服务启动")
    url="https://api.telegram.org/bot{tt}/sendMessage".format(tt=TG_BOT_TOKEN)
    headers = {'Content-Type': 'application/x-www-form-urlencoded'}
    payload = {'chat_id': str(TG_USER_ID), 'text': '{title}\n\n{content}'.format(title = title,content = content), 'disable_web_page_preview': 'true'}
    proxies = None
...
def dingding_bot(title, content):
    ...
    url = 'https://oapi.dingtalk.com/robot/send?access_token={a1}&timestamp={b1}&sign={c1}'.format(a1 =DD_BOT_ACCESS_TOKEN ,b1 =timestamp,c1 =sign )
...
def coolpush_bot(title, content):
    ...
    if not QQ_SKEY or not QQ_MODE:
        ...
        return
    print("qq服务启动")
    url="https://qmsg.zendee.cn/{qq}/{qq1}".format(qq = QQ_MODE,qq1 =QQ_SKEY)
    payload = {'msg': "{title}\n\n{content}".format(title=title,content=content).encode('utf-8')}
